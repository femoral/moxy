...
const makeJsonCreateCollectionRepository =
  ({ collectionsBasePath }: { collectionsBasePath: string }): CreateCollection =>
  async (collection: Collection): Promise<void> => {
    ...
    if (existsSync(collectionFolder)) {
      ...
      await fs.writeFile(path.join(collectionFolder, 'collection.json'), JSON.stringify(collectionModel, undefined, 4));
    }
  };
