...
export const makeAddCollectionController =
  ({ createCollectionUseCase }: { createCollectionUseCase: CreateCollectionUseCase }): RequestHandler =>
  async (req: Request, res: Response) => {
    const collectionDto: CollectionDto = req.body;
    const collection = await createCollectionUseCase.execute(collectionMapper.map(collectionDto));
